...
def usage():
    """Print the usage."""
    print("\nkilda.py -c [counts_list.tsv] -k [KIV2_hg38_kmers_6copies_specific.tsv] -l [LPA_hg38_kmers_1copies_specific.tsv] -r [lpa_rsids.tsv] -o [output_folder] -p -v")
    print("\nInput:")
    ...
    print("     -k/--kiv             File listing the curated KIV2 kmers. (required, default = './KIV2_hg38_kmers_6copies_specific.tsv')")
    print("     -l/--lpa             File listing the curated Normalisation kmers. (required, default = './LPA_hg38_kmers_1copies_specific.tsv')")
    print("     -r/--rsids           File listing rsids of interest and their ref and alt kmers. (optional, tab-delimited, no header)")
    print("     -q/--quantiles       File listing quantiles values, samples will be assigned to quantiles based on this list (optional, if not present, quantiles will be computed from the cohort itself)")
    print("\nOutput:")
...
def parse_arguments():
    ...
    kiv_kmers_file = "./KIV2_hg38_kmers_6copies_specific.tsv"
    lpa_kmers_file = "./LPA_hg38_kmers_1copies_specific.tsv"
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "c:k:l:r:q:o:pvhV", ["counts=", "kiv=", "lpa=", "rsids=", 
                                                                      "quantiles=", "output=", "plot", "verbose", 
                                                                      "help", "version"])
    ...
    return counts_list_file, kiv_kmers_file, lpa_kmers_file, rsids_kmers_file, quantiles_file, output_folder, plot_on, verbose_on


def check_arguments(counts_list_file, kiv_kmers_file, lpa_kmers_file, rsids_kmers_file, quantiles_file, output_folder):
    ...
    Args:
        ...
        quantiles_file: a string containing the path to the file with quantiles of interest
    ...
    if(quantiles_file != None):
        if not os.path.isfile(quantiles_file):
            sys.exit("Error for option -q/--quantiles: '"+str(quantiles_file)+"' is not a file.")
...
def read_quantiles_file(quantiles_file, verbose_on):
    """From a file containing a list of values (corresponding to the limits of pre-determined quantiles),
    creates a list containing the sorted values (to later insert the KIV-2 CNs, and get the quantile from the index).

    Args:
        quantiles_file: a string containing the path to the file with the quantiles
        verbose_on: A boolean indicating if messages must be printed or not

    Returns:
        A list with the sorted quantiles values
    """
    quantiles_df = pd.read_csv(quantiles_file, names = ['quantiles'], header = None)

    if(quantiles_df.isnull().values.any()):
        sys.exit("Error when reading '"+quantiles_file+"': some values are null. Is the format of the file correct? Should only contains the start values of each quantile & no header.\nStopping now...")
    
    if not quantiles_df["quantiles"].is_unique:
        sys.exit("Error: quantiles values are not unique in '"+ quantiles_file +"'. Stopping now.")

    if(verbose_on): print("Read '"+str(quantiles_df.shape[0])+"' quantile intervals from "+quantiles_file+".\n")

    quantiles_list = quantiles_df['quantiles'].tolist()
    quantiles_list.sort()
...
def main():
    """The main function"""
    counts_list_file, kiv_kmers_file, lpa_kmers_file, rsids_kmers_file, quantiles_file, output_folder, plot_on, verbose_on = parse_arguments()
    check_arguments(counts_list_file, kiv_kmers_file, lpa_kmers_file, rsids_kmers_file, quantiles_file, output_folder)
    ...
    # If a quantile file was inputted, we use it, else we will compute them from the cohort directly:
    if(quantiles_file != None):
        qbins = read_quantiles_file(quantiles_file, verbose_on)
    ...
    if(quantiles_file != None):
        qbins.insert(0, 0)
        qbins.append(float('inf'))
        samples_df['quantile'] = pd.cut(samples_df['KIV2_CN'], qbins, labels = False, duplicates = 'raise')
    else:
        try:
